...
def show_status():
    ...
    if equal_supplies:
        print_at("Supplies collected:  {0} / {1}".format(my_building.player_food, player_needs),
                 screen_size[1] + 15, 20)
    else:
    ...
    if event.type == pygame.KEYDOWN:
        if ((event.key == pygame.K_LEFT and my_building.move_player(0, -1)) or
                ...
                if not my_building.can_move():
                    ...
                    msg = "You collected supplies for {0} out of {1} days. You {2}."\
                        .format(supplies, player_needs, "survive" if victory else "die")
                    if victory:
